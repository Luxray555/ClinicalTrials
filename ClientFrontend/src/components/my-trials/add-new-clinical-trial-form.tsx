...
import addNewClinicalTrial from "@/actions/investigator/add-new-clinical-trial";
...
export default function AddNewClinicalTrialForm({
  setAddNewClinicalTrialOpen,
}: {
  setAddNewClinicalTrialOpen: React.Dispatch<React.SetStateAction<boolean>>;
}) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    setError("");
    try {
      const data = await addNewClinicalTrial({
        ...values,
        dates: {
          startDate: format(values.dates.startDate, "yyyy-MM-dd"),
          estimatedCompletionDate: format(
            values.dates.estimatedCompletionDate,
            "yyyy-MM-dd",
          ),
        },
        sourceMetaData: {
          originalSourceId: Math.random().toString(36).substring(7),
          trialSourceUrl: "",
          dataSource: {
      ...
      if ("error" in data) {
        ...
        setAddNewClinicalTrialOpen(false);
        toast({
  ...
  return (
    <Form {...form}>
      ...
      >
        ...
        <Button type="submit">
          {isLoading ? <LoadingSpinner /> : "Add clinical trial"}
        </Button>
      </form>
    </Form>
  );
